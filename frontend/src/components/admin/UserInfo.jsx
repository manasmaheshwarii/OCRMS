...
const UserInfo = () => {
  ...
  const handleSubmit = async (user_id) => {
    if (updateUser === "") {
      ...
      if (confirmed) {
        axios
          .put(`${process.env.REACT_APP_API_URL}/user/${user_id}`, updateUser)
          .then((res) => {
    ...
    const getOrdinaryRecords = async () => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_API_URL}/OrdinaryUsers`
        );
      ...
      buttons: [
        {
          ...
          onClick: async () => {
            try {
              await axios.delete(
                `${process.env.REACT_APP_API_URL}/OrdinaryUsers/${userId}`
              );
              setOrdinaryList(
                ordinaryList.filter((user) => user._id !== userId)
              );
