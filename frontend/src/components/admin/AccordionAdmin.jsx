...
const AccordionAdmin = () => {
  ...
  useEffect(() => {
    ...
    const getAgentsRecords = async () => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_API_URL}/AgentUsers`
        );
  ...
  const handleSelection = async (agentId, complaintId, status, agentName) => {
    try {
      await axios.get(`${process.env.REACT_APP_API_URL}/AgentUsers/${agentId}`);
      ...
      await axios.post(
        `${process.env.REACT_APP_API_URL}/assignedComplaints`,
        assignedComplaint
      );
      const updatedComplaintList = complaintList.filter(
